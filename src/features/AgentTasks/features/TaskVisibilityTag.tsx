...
const TaskVisibilityTag = memo<TaskVisibilityTagProps>(
  ({
    ...
    const label = t(
      ...
      visibility === 'private' ? { defaultValue: 'Private' } : { defaultValue: 'Workspace' },
    );
      ...
      () =>
        VISIBILITY_OPTIONS.map((option, index) => {
          ...
          return {
            ...
            key: option,
            label: t(
              `createTask.visibility.${option}` as never,
              option === 'private' ? { defaultValue: 'Private' } : { defaultValue: 'Workspace' },
            ),
